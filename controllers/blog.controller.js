...
export const getMyTotalBlogLikes = async (req, res) => {
  try {
    ...
    const myBlogs = await Blog.find({ author: userId }).select("title likes");
    ...
    const totalLikes = blogs.reduce((acc, b) => acc + b.likesCount, 0);
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch total blog likes" });
  }
};
